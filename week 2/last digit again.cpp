...
int main() {
	...
	cin >> m >> n ;
	vector<long> fibs;
	fibs = get_pisano_period(static_cast<long>(10));
	long sum = 0;
	long true_m = m % 60, true_n = n % 60;
	if (true_m == true_n ){
		cout << get_fib(true_m) % 10;
		return 0;
	}
	if (true_n - true_m > 0){
		if (true_m == 0) true_m++;
		for (int i = true_m -1; i < true_n; i++){
			sum += fibs[i];
		}
		cout << sum % 10;
	}
	else {
		for (int i = true_m -1; i < fibs.size(); i++){
			sum += fibs[i];
		}
